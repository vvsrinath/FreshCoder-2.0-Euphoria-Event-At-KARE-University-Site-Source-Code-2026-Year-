...
async function updateQuestion(ctx: RouteCtx): Promise<HttpResponse> {
  ...
  await execute(
    ...
    [
      version,
      str(body["title"] ?? String(body["question"] ?? body["prompt"] ?? row["prompt"] ?? "").slice(0, 80)).slice(0, 500),
      qtype,
      str(body["topic"] ?? row["topic"]).slice(0, 100),
      str(body["difficulty"] ?? row["difficulty"]),
      marks,
      body["status"] !== undefined ? body["status"] : row["status"],
      body["question"] !== undefined ? body["question"] : body["prompt"] !== undefined ? body["prompt"] : row["prompt"],
